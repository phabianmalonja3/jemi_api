...
public class BookingController {
  ...
  public ResponseEntity<?> createBooking(
      @Valid @RequestBody BookingDTO bookingDTO, Principal principal) {
    var savedBooking = bookingService.saveBooking(bookingDTO, principal.getName());
    return new ResponseEntity<>(savedBooking, HttpStatus.CREATED);
  }

  @GetMapping("/me")
  public ResponseEntity<List<BookingDTO>> getClientBooking(Principal principal) {
  ...
  public ResponseEntity<?> checkAvailability(
      ...
      @RequestParam LocalDateTime pickupTime) {

    boolean available = bookingService.checkAvailability(photographerId, packageId, pickupTime);
  ...
  public ResponseEntity<BookingResponseDTO> updateStatus(
      @PathVariable UUID id, @RequestParam BookingStatus status) {
  ...
  public ResponseEntity<?> getActiveBooking(Principal principal) {
    Optional<Bookings> activeBooking =
        bookingService.getActiveBookingForClient(principal.getName());
