...
public class LiveStatusService {
  ...
  private static @NonNull String getString(GeoResult<RedisGeoCommands.GeoLocation<Object>> result) {
    double distanceInKm = result.getDistance().getValue();
  ...
  public Map<String, Object> toggleStatus(String email, LocationUpdateRequest request) {
    ...
    return Map.of(
        ...
        user.getIsOnline(),
        "status",
        request.isOnline() ? "LIVE" : "OFFLINE");
  }

  public void broadcastPhotographerLocation(
      UUID userId, double lat, double lng, String name, String profileImage) {
    Map<String, Object> locationUpdate =
        Map.of(
    ...
    messagingTemplate.convertAndSend("/topic/photographers/locations", Optional.of(locationUpdate));
  }
  ...
  public Map<String, Object> renewHeartbeat(String email, LocationUpdateRequest request) {
    ...
    if (request.getLongitude() == null || request.getLatitude() == null) {
      throw new IllegalArgumentException(
          "Longitude and Latitude are required to renew live session");
    }
    ...
    return Map.of(
        ...
        35,
        "message",
        "Heartbeat renewed successfully. Live session extended.");
  }
