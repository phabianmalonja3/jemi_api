...
public class BookingServiceImp implements BookingService {
  ...
  public List<BookingDTO> getBookingsByPhotographerEmail(String email) {
    return bookingRepository.findByPhotographerEmail(email).stream()
  ...
  public BookingDTO saveBooking(BookingDTO dto, String email) {
    ...
    User photographer =
        userRepository
            .findById(dto.getPhotographerId())
            .orElseThrow(() -> new RuntimeException("Photographer not found."));
    Pkg pkg =
        pkgRepository
            .findById(dto.getPackageId())
            .orElseThrow(() -> new RuntimeException("Package not found."));

    // 1. Define start time (pickupTime)
    LocalDateTime startTime =
        dto.getPickupTime() != null ? dto.getPickupTime() : LocalDateTime.now();
    int packageHours =
        (pkg.getDuration() != null) ? pkg.getDuration() : 1; // default to 1 hour if null
    LocalDateTime endTime = startTime.plusHours(packageHours);

    Bookings booking =
        Bookings.builder()
            .client(client)
            .photographer(photographer)
            .amountPaid(BigDecimal.valueOf(pkg.getPrice()))
            .pkg(pkg)
            .type(BookingType.valueOf(dto.getType()))
            .pickupTime(startTime)
            .endTime(endTime) // Stored so future lookups can catch overlaps
    ...
    String photographerPhone =
        (photographer.getUserProfile() != null) ? photographer.getUserProfile().getPhone() : null;
    String clientAvatar = client.getProfileImageUrl();
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm");
    String formattedPickupTime =
        (savedBooking.getPickupTime() != null)
  ...
  public Bookings updateBookingStatus(UUID bookingId, BookingStatus newStatus) {
    ...
    String clientPhone =
        booking.getClient().getUserProfile() != null
            ? booking.getClient().getUserProfile().getPhone()
            : null;
    eventPublisher.publishEvent(
  ...
  public Optional<Bookings> getActiveBookingForClient(String email) {
    ...
    List<BookingStatus> finishedStatuses =
  ...
  public List<BookingDTO> getBookingsByClientEmail(String email) {
    User client =
        userRepository
            .findByEmail(email)
            .orElseThrow(
                () -> new RuntimeException("Client account not found with email: " + email));
  ...
  @Override
  @Transactional
  public boolean checkAvailability(UUID photographerId, UUID packageId, LocalDateTime pickupTime) {
    User photographer =
        userRepository
    ...
    int packageHours = pkg.getDuration() != null ? pkg.getDuration() : 1;

    LocalDateTime endTime = pickupTime.plusHours(packageHours);

    boolean isBusy =
        bookingRepository.existsConflictingBooking(photographer.getId(), pickupTime, endTime);
